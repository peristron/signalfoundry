...
This README describes the **lighter** Signal Foundry app branch: the general-purpose text analytics version. A heavier branch may include a larger TAM-specific maturity module, but that specialized module is not the focus of this README.
...
Before scanning, the app shows a **Pre-Scan Data Preview** with:
...
After scanning, the app shows a **Latest Scan Summary** with:
...
3. Review the **Pre-Scan Data Preview**.
...
6. Run the scan.
7. Review the **Latest Scan Summary**.
...
Batch scan mode uses a safe default for CSV files: the first detected column is used unless you scan a file individually and choose specific text columns.
...
### Path D: Offline harvester workflow
...
The word cloud and frequency tables use the same cleaned token stream. If these look wrong, downstream outputs such as topics, graph relationships, sentiment, and maturity scoring should be treated cautiously until cleaning settings are corrected.
...
The lighter app includes a simpler maturity feature. A separate heavier app branch may include a larger TAM-specific maturity module.
...
- Confirm the pre-scan preview appears before scanning.
- Confirm the latest scan summary appears after scanning.
...
Batch scan mode uses the first detected CSV column as a safe default. For multi-column CSVs, scan the file individually and select the intended text column or columns.
